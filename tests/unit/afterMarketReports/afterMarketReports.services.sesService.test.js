...
const sesService = require('../../../lambda/afterMarketReports/services/sesService.js').sendEmail;

describe('sesService', () => {

    let messageIds;
    let activeSubscriptions= ['testgmail.com'];

    beforeAll(async () => {

        // run live tests on dev only
        if(STAGE === 'dev') {
            messageIds = await sesService(activeSubscriptions);
        } else {
